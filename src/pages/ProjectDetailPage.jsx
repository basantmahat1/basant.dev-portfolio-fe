  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-5xl px-6 py-8">
        ...
        <Link
          to="/projects"
          className="mb-6 inline-flex items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--panel)] px-4 py-1.5 text-xs font-semibold text-tertiary transition hover:bg-tertiary hover:text-white"
        >
        ...
        <motion.div
          ...
          className="shell mb-12"
        >
          <div className="glass overflow-hidden p-6 md:p-8">
            {/* Banner Image */}
            {heroImage && (
              <div className="relative mb-8 aspect-[16/8] w-full overflow-hidden rounded-2xl border border-[var(--border)] shadow-md">
                <img
                  src={heroImage}
                  alt={project.title}
                ...
                <div className="absolute bottom-4 left-4 flex flex-wrap items-center gap-2">
                  <span className="rounded-full bg-tertiary px-3 py-1 text-xs font-bold text-white shadow-sm">
                    {project.category}
                  </span>
            ...
            <h1 className="mb-4 font-display text-3xl font-extrabold leading-tight text-text-primary md:text-4xl">
              {project.title}
            </h1>
            ...
            <div className="mb-8 flex flex-wrap gap-2">
              {(project.technologies || []).map((tech) => (
                <span
                  key={tech}
                  className="rounded-full border border-[var(--border)] bg-[rgba(249,238,217,0.5)] px-3 py-1 text-xs font-semibold text-text-primary"
                >
                  {tech}
                </span>
              ))}
        ...
        {(project.clientName || project.myRole || project.teamSize || project.completionDate) && (
          <motion.div {...fadeUp} className="shell mb-12">
            <div className="glass grid grid-cols-2 gap-4 p-6 md:grid-cols-4">
              {project.clientName && (
                ...
                <div className="shell">
                  <div className="glass h-full p-6 md:p-8">
                    <div className="mb-4 flex items-center gap-2.5 text-amber-600 dark:text-amber-400">
                      ...
                      <h3 className="font-display text-lg font-bold text-text-primary">The Challenge</h3>
                    </div>
                ...
                <div className="shell">
                  <div className="glass h-full p-6 md:p-8">
                    <div className="mb-4 flex items-center gap-2.5 text-emerald-600 dark:text-emerald-400">
                      ...
                      <h3 className="font-display text-lg font-bold text-text-primary">The Solution</h3>
                    </div>
              ...
              {deepDiveItems.map(({ key, title, icon: Icon, content }) => (
                <div key={key} className="shell">
                  <div className="glass h-full p-6">
                    <div className="mb-3 flex items-center gap-2.5 text-tertiary font-semibold text-base font-display">
                      <Icon size={16} />
                      {title}
                    </div>
          ...
          <motion.section {...fadeUp} className="mb-12">
            <div className="grid gap-6 md:grid-cols-2">
              {project.details?.lessonsLearned && (
                <div className="shell">
                  <div className="glass h-full p-6">
                    <h3 className="mb-3 font-display text-base font-bold text-text-primary flex items-center gap-2">
        ...
        <div className="mb-16 flex items-center justify-between border-t border-[var(--border)] pt-8">
          {prev ? (
            <Link to={`/projects/${prev.slug}`} className="group flex items-center gap-2 text-sm font-semibold text-text-primary hover:text-tertiary">
              <FaArrowLeft size={12} className="transition group-hover:-translate-x-1" /> {prev.title}
            </Link>
          ) : (
            <span />
          )}
          {next && (
            <Link to={`/projects/${next.slug}`} className="group flex items-center gap-2 text-sm font-semibold text-text-primary hover:text-tertiary">
              {next.title} <FaArrowRight size={12} className="transition group-hover:translate-x-1" />
            </Link>
          )}
        </div>
      </div>
